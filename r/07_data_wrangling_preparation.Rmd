...
## Data Management
  ...
  Data are often extracted directly from the source systems in which they are generated or downstream environments such as a *data lake*, *data warehouse*, or *data mart*.
...
```
  ...
  A DW usually contains many different types of tables, but the two most common are *Type 1* and *Type 2* tables. A **Type 1 table** is overwritten on a regular cadence (usually daily) and contains no history -- only current values. For example, a Type 1 table may contain the latest known attributes for each active and terminated worker such as job, location, and manager. A Type 1 table is convenient when only current information needs to be queried. A **Type 2 table**, also known as a **slowly changing dimension (SCD)**, is a table in which a new record is inserted when a change occurs for one or more specified dimensions. Jobs, locations, and managers are examples of slowly changing dimensions but unlike the Type 1 table which is truncated and reloaded with the latest information, the Type 2 table houses a *start date* and *end date* for each worker and dimension to facilitate reporting and analysis on changes to attribute values over time.
  
  Figure \@ref(fig:type-2-tbl) illustrates the design of a Type 2 SCD for a worker's job changes. Note that for current values, the end date is usually set to `12/31/9999` and accompanied by a current record flag:
  
```{r type-2-tbl, out.width = "100%", echo = FALSE, fig.cap = 'Type 2 SCD with Job Changes', fig.align = 'center'}
