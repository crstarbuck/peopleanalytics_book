...
## SQL
  ...
  * **ORDER BY**: Specifies how data should be sorted in the output
  ...
  First, let's load the `sqldf` library and load the data sets:
...
```
  ...
  **Aggregate Functions**
  ...
  Next, let's take a look at the average organization tenure by job for those in the Research & Development department:
...
```
  ...
  There are 6 distinct job titles among employees in the Research & Development department, and the average organization tenure for these ranges from `5` to `13.7` years.
  
  Since there could be few employees in certain jobs, in which case average organization tenure may not be as meaningful, we can use the `COUNT(*)` function to count the number of rows for each group. In this case, `COUNT(*)` will return the number of employees in each job in the Research & Development department. We will also utilize aliases via `AS` in the `SELECT` clause to assign different names to the output fields:
...
```
  ...
  The output shows that there are only 2 Vice Presidents in the Research & Development department, while other job titles are much more common. 
  
  Since relatively few employees are Vice Presidents, let's use the `HAVING` clause to only show average organization tenure for Research & Development department jobs with more than 10 employees:
...
```
  ...
  Next, we will query these data frames to return the average organization tenure and average commute distance for employees in the Research & Development department, grouped by jobs with more than 10 employees. For this, we will leverage an `INNER JOIN`, which will return records only for employee ids which are present in all three data frames. For example, if a record exists in `demographics` and `tenure` for a particular employee id, but there is no team designation for it in `job`, that employee id would not be included in the output.
...
```
  ...
  In this case, if demographics is the base data set which contains all employee ids, it is important for `tenure` and `job` to be joined to it. Joining `job` to `tenure` may result in information loss if an employee id exists in `demographics` and `job` but not in the intermediate `tenure` data set.

  When integrating data within R, the `tidyverse` provides a more parsimonious method of joining many data sets using various join types. The example below joins nine data sets into a single `employees` data frame using a left join on the `employee_id` column:
...
```
  ...
  **Subqueries** are queries nested within other queries. Subqueries are often referred to as **inner queries**, while the main queries are referred to as **outer queries**.
  ...
  For example, if we are interested in performing an analysis on employees with more than a year of organization tenure, we can use a subquery to pass a list of employee ids that meet this criteria into the outer query for filtering. In this case, we would not need to include `tenure` in the join conditions of our main query:
...
```
  ...
  **Virtual Tables**
  ...
  An alternative to a subquery is creating a **virtual table** in the `FROM` clause using a SQL statement. When using an `INNER JOIN` to connect `demographics` to the virtual table containing a list of employee ids for those with more than a year of organization tenure, any records in `demographics` or `job` that do not relate to employees with at least a year of organization tenure will be dropped. With this approach, our `WHERE` clause is limited to the `department = 'Research & Development'` condition:
...
```
  ...
  Notice that in the case of organization tenure ties, the `RANK()` function assigns the same rank and then adds the number of ties to that rank to determine the rank for the next highest value of commute distance.
