...
```
  ...
  In this example, the median is only `3` while the mean is `87`. Large deltas between mean and median values provide important information about the distribution of data. 
  
  In this example, a single value has significant leverage on these measures of central tendency. To demonstrate, let's eliminate one instance of `3` from the vector and recalculate the mean and median:
...
```
  ...
  By removing a single value from this vector, the mean increased from `87` to `101` and the median from `3` to `51.5`! 
  
  In some cases, outliers may be the cause of large discrepancies between mean and median values since the mean can be sensitive to extreme values. However, differences in mean and median values for `x` and `x1` in our example are *not* due to an extreme value (outlier), as `3` is similar to half of the values in the vector. This highlights the importance of understanding the data distribution and evaluating *both* the mean and median. 
