...
```{r, echo = FALSE, fig.cap = 'Table of Correlation Coefficients', fig.align = 'center'}
...
```
  ...
  The Phi Coefficient ($\phi$), sometimes referred to as the mean square contingency coefficient, can be used to understand the association between two dichotomous variables. For this, let's examine whether there is a relationship between gender and performance after transforming performance from its ordinal form to a dichotomous variables (high vs. low performance). We will leverage the `psych` library to implement this in R:
