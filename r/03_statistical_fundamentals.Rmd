...
var(demographics$annual_base_compensation)
...
var(demographics$annual_base_compensation) * (n - 1) / n
...
sd(demographics$annual_base_compensation)
...
sd(demographics$annual_base_compensation) * sqrt((n - 1) / n)
...
quantile(demographics$annual_base_compensation)
...
quantile(demographics$annual_base_compensation, probs = .8)
...
summary(demographics$annual_base_compensation)
...
# Calculate covariance between Belonging and Culture scores using complete observations (missing values are present in the data)
cov(responses$belonging, responses$culture, use = "complete.obs")

```
  
  In this example, using the default Pearson method, the covariance between two variables measured on 5-point Likert scales is .35. However, given the ordinal nature of this data, we should calculate covariance using the Spearman method. The Pearson method should be used when working with continuous data (e.g., compensation, age), and the Spearman method when working with rank-ordered variables (e.g., Likert scales, numeric education levels). We can override the default method using the method() argument in the cov() function call:
