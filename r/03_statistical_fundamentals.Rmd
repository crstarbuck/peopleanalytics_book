...
```
  ...
  Now, let's illustrate the relationship between CIs and standard errors using sample data from our uniform population distribution. In our example, both $\mu$ and $\sigma$ are known and our sample size $n$ is at least 30; therefore, we can use a Z-Test to calculate the 95% CI. A $z$ score of 1.96 corresponds to the 95% CI -- the range of values we would expect to include the population mean $\mu$ in at least 95 of 100 random samples taken from the population:
...
```
  ...
  Our known population mean $\mu$ is 51.2, which is covered by our 95% CI (47.9 - 59.0). Per the CLT, in less than 5% of cases would we expect to draw a random sample from the population that results in a 95% CI which does not include the population mean. Note that our CI narrows with larger samples since our confidence that the range includes the population average increases with more data.
...
```
  ...
  Like the 95% CI, this slightly wider 99% CI (46.2 - 60.7) also includes the true population mean of 51.2.
  ...
  As we will see, the T-Test underpins many statistical tests and models germane to the people analytics discipline, so it is important to understand the mechanics. The $t$ distribution is wider and shorter relative to the normal distribution, and this is characteristic of the distribution of sample means for smaller samples we observed in our CLT example. As a result, we must apply a different factor to estimate the CI to reflect the more *platykurtic* distribution. The CI in this case is defined by:
