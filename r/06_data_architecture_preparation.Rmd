# Data Management & Preparation {#data-arch-prep}

  To begin a data analysis, we must first access, organize, and clean the requisite data. As depicted in Figure \@ref(fig:ds-tasks), these tasks account for a large part of the work analytics professionals do.
...
## Data Architecture
  ...
  To properly and efficiently extract data -- often through the use of SQL as covered in Chapter \@ref(sql-intro) -- it is important to first understand some of the most common ways in which data are stored and structured. Data are generally extracted directly from the source systems in which they are generated or from downstream repositories such as a *data lake*, *data warehouse*, or *data mart*.
  ...
  It is important to note that leveraging snapshots for trending analyses has significant deficiencies given the large number of retroactive transactions processed in Human Capital Management (HCM) systems that are not captured if prior snapshots are not updated. Below are some examples:
