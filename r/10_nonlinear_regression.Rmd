...
```{r glm, out.width = "100%", echo = FALSE, fig.cap = 'Logistic regression of active status onto interview rating.', fig.align = 'center'}
...
```
  ...
  The output shown in Figure \@ref(fig:glm) has some differences relative to the output of a linear regression model. 
...
```{r lm-glm-compare, out.width = "100%", echo = FALSE, fig.cap = 'Linear (left) and logistic (right) functions applied to models regressing active status (1/0) onto median interviewer rating.', fig.align = 'center'}
...
```
  ...
  It is often helpful to explain the relationship between a binary predictor and binary outcome in terms of a percentage increase or decrease. When $\beta = 1$, this indicates that the likelihood of the outcome is identical between the two groups of the predictor (e.g., attrition likelihood for remote and non-remote employees). Therefore, $(\beta - 1) * 100$ will provide the percentage increase or decrease in the odds of the included group (dummy coded as 1) relative to the omitted group (dummy coded as 0). For example, if $\beta = 2.75$, there is a $(2.75 - 1) * 100 = 175$ percent increase in the odds. If $\beta = .95$, there is a $(1 - .95) * 100 = 5$ percent decrease in the odds.
