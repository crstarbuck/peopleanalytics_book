...
```  
  ...
  A **Paired Samples $t$-Test** is used to compare means between pairs of measurements. This test is known by many other names, such as a **dependent samples** $t$**-test**, **paired-difference** $t$**-test**, **matched pairs** $t$**-test**, and **repeated-samples** $t$**-test**. We can run a paired samples $t$-test in R by passing `paired = TRUE` as an argument to the same `t.test()` function used for the independent samples $t$-test. 
