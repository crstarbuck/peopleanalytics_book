...
``` 
  ...
  Even with these transformations, there is still a clear S-shaped curve about the residuals. Though we cannot proceed with ANOVA due to violated assumptions, the implementation of ANOVA involves the `aov()` function along with the `summary()` function to display model output:
...
# ANOVA investigating interaction between mean differences in annual comp by job satisfaction x stock option level
