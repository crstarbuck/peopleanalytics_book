...
## Parametric vs. Nonparametric Tests
  ...
  In the context of continuous (quantitative) data, we will cover parametric tests along with their nonparametric counterparts. When the hypothesis relates to average (mean) differences and $n$ is large, **parametric tests** are preferred as they generally have more statistical power. However, the assumptions of parametric tests are not always met.
  
  **Nonparametric tests** are **distribution-free tests** that do not require the population's distribution to be characterized by certain parameters, such as a normal distribution defined by a mean and standard deviation. While useful when data are nonnormal, it is important to note that nonparametric tests are not only less powerful than their parametric equivalents, they also require hypothesis modification since most nonparametric tests are about the *median* rather than *mean* centers.
...
```{r continuous-tests, out.width = "75%", echo = FALSE, fig.cap = 'Continuous Data: Parametric & Nonparametric Tests of Differences', fig.align = 'center'}
...
```{r discrete-tests, out.width = "75%", echo = FALSE, fig.cap = 'Discrete Data: Criteria for Chi-Square & Fisher Exact Tests', fig.align = 'center'}
