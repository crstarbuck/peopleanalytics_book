...
```
  ...
  The `replace = F` argument in the sample() function indicates that we want to sample *without* replacement (i.e., we do not want an employee to be selected twice in the sample). For example, if we draw multiple names from a hat without replacement, we would not put names back into the hat once they are drawn; each has a chance of being selected only once.
