...
```
  ...
  Let's compare CIs calculated using a T-Test to those calculated using the Z-Test. While a fixed $z$ score can be used for each CI level when $n$ > 30, the $t$ statistic varies based on both the CI level and $df$. Though R will determine the correct $t$ statistic for us, let's reference the table shown in Figure \@ref(t-crit) to manually lookup the $t$ statistic:
  
```{r t-crit, echo = FALSE, fig.cap = paste("Critical Values of Student's t Distribution"), fig.align = 'center'}
