...
```
  ...
  * **Step 1**: Partition data into training and test sets for cross-validation.
  ...
  Since we have 60 months of data for each combination of `job`, `level`, and `remote` in the data, we will select combinations for which `turnover_rate` can be projected for future months. To simplify, we will train models using data for the first 48 months and test using the final 12 months.
...
```
  ...
  * **Step 2**: Build models using training data.
  ...
  Given the significant quadratic and cubic terms identified in Chapter \@ref(lme), we will fit cubic regression models on the training data.
  
```{r, message = FALSE, warning = FALSE}

# Fit cubic models
train.cube.fit.1 <- lm(turnover_rate ~ year + month + I(month^2) + I(month^3) + remote, data = train_dat_1)
train.cube.fit.2 <- lm(turnover_rate ~ year + month + I(month^2) + I(month^3) + remote, data = train_dat_2)
train.cube.fit.3 <- lm(turnover_rate ~ year + month + I(month^2) + I(month^3) + remote, data = train_dat_3)

```

```{r train-cube-fit-1, out.width = "100%", fig.cap = 'Cubic model output for regression of turnover rate onto year, month, and remote for training set 1.', fig.align = 'center', message = FALSE, warning = FALSE}

# Produce tabular summary of regression model output
flextable::as_flextable(train.cube.fit.1)

```

```{r train-cube-fit-2, out.width = "100%", fig.cap = 'Cubic model output for regression of turnover rate onto year, month, and remote for training set 2.', fig.align = 'center', message = FALSE, warning = FALSE}

# Produce tabular summary of regression model output
flextable::as_flextable(train.cube.fit.2)

```

```{r train-cube-fit-3, out.width = "100%", fig.cap = 'Cubic model output for regression of turnover rate onto year, month, and remote for training set 3.', fig.align = 'center', message = FALSE, warning = FALSE}

# Produce tabular summary of regression model output
flextable::as_flextable(train.cube.fit.3)

```

  As expected, all linear, quadratic, and cubic terms are statistically significant.
