...
```
  ...
  Let's compare the information from the RF's Variable Importance Plot to a single decision tree built on training data with balanced classes. We can build and visualize a decision tree in R using the `rpart()` and `rpart.plot()` functions:
...
```  
  ...
  As shown in Figure \@ref(fig:rpart-tree), the only predictor on which the data are split is `interview_rating` when using a single decision tree. At the root node, there is a 50 percent chance of leaving, which is expected given we balanced the classes using SMOTE. Among employees with `interview_rating >= 3.5`, there is a 1 percent chance of leaving and among employees with `interview_rating < 3.5`, there is a 99 percent chance of leaving. Given the excellent performance splitting data only on `interview_rating`, no further partitioning is necessary since it would likely result in modeling noise and overfitting.
