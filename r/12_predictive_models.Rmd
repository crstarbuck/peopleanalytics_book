...
# Note: A loop is not the most efficient, but it is leveraged here to simplify instruction on SMOTE mechanics
for (i in 1:training_class_delta){
  
  # Sample employee id from underrepresented class
  oversampled_id <- sample(training_dat[training_dat$active == 1, 'employee_id'], size = 1, replace = TRUE)
  
  # Store observation for sampled employee id
  new_obs <- unique(training_dat %>% filter(employee_id == oversampled_id))
  
  # Append sampled observation to training data frame
  training_dat <- rbind(training_dat, new_obs)
}

# Return Boolean to validate that classes are equal in the training data
nrow(training_dat %>% filter(active == 0)) == nrow(training_dat %>% filter(active == 1))

```

  Next, we will fit a binomial logistic regression model using a subset of predictors from the oversampled training data:

```{r glm-train-output, out.width = "100%", echo = FALSE, fig.cap = 'Binomial logistic regression model output for regression of active status onto overtime, job level, engagement, and interview rating.', fig.align = 'center', message = FALSE, warning = FALSE}
...
```
  ...
  As we can see, results of the binomial logistic regression model indicate that all predictors are important in classifying employees into active and inactive classes for these training data.
...
rf.fit <- randomForest(active ~ overtime + job_lvl + engagement + interview_rating, data = training_dat)
...
```
  ...
  We can use the `predict()` function in conjunction with an object holding the trained model to predict values for the test data. We will store and label model performance metrics in a single data frame for easy comparison:
...
perf.metrics <- rbind(perf.metrics, cbind.data.frame(
  val = assess.prediction(test$CLASS, ifelse(predict(glm.fit, test, type = "response") > .5, 1, 0)),
  method = paste0("LOG")))

# Predict with RF model
perf.metrics <- rbind(perf.metrics, cbind.data.frame(
  val = assess.prediction(test$CLASS, predict(rfRes,subset(test, select = predictors))),
  method = paste0("RF")))
...
```{r class-mdl-perf, out.width = "100%", echo = FALSE, fig.cap = '.', fig.align = 'center', message = FALSE, warning = FALSE}

ggplot(perf.metrics, aes(x = method, y = val.accuracy, fill = method)) + geom_bar(stat="identity") + geom_text(data = perf.metrics, aes(label = val.accuracy), vjust = 1.5, colour = "white", size = 5) + labs(x = "Method", y = "Accuracy (%)", title = "Accuracy by Model Type")
