...
## Visualization Types
  ...
  There are some highly advanced and interactive data visualizations that can be built using Javascript and D3 libraries. While these are beyond the scope of this book, `ggplot2` is capable of building some very elegant data visualizations, and this will be the tool used to implement each of the data visualizations covered in this section.

  All visuals will have a set of common parameters for design aesthetics. Therefore, each section will highlight the differences needed to achieve the respective data viz type. We could wrap common `ggplot2` elements in a function to simplify chart building. 
...
### Heatmaps
  ...
  **Heatmaps** use a shading scheme to highlight the relative magnitude of numbers in a tabular format.
  ...
  To demonstrate the implementation of heatmaps in R, let's construct a simple cube containing employee counts by department and tenure band:

```{r, message = FALSE, warning = FALSE}

# Append new tenure band column
employees$tenure_band <- case_when(
...
```
  ...
  The `geom_tile()` function can be used to build a heatmap. The range of colors can be specified via the `scale_fill_continuous(low = 'minimum value color', high = 'maximum value color')` function since the fill variable defined by `aes(fill = cnt)` is measured on a continuous scale:
...
### Scatterplots
  ...
  **Scatterplots** are useful for visualizing relationships between numeric variables.
  ...
  Let's first subset `employees` data to active salespeople and append a flag to indicate sales quota attainment ($150,000 USD$) for use in shading data points in the scatterplot.
...
```{r scatterplot, out.width = "100%", fig.cap = "Scatterplot showing the relationship between work experience and YTD sales for active salespeople. Data point colors are relative to the full year sales quota of USD 150k.", fig.align = 'center', warning = FALSE, message = FALSE}

require(scales)

ggplot2::ggplot(sales, aes(x = work_exp, y = ytd_sales, group = quota_flg)) +
ggplot2::geom_point(aes(color = as.factor(quota_flg))) +
ggplot2::geom_hline(yintercept = 150000, linetype = 2, color = "#0070C0") + 
ggplot2::annotate(geom = "text", x = 2, y = 159000, label = "Sales Quota = $150k", color = "#0070C0", size = 3) +
ggplot2::labs(title = 'Some employees with significant work experience have \nalready met the full year sales quota.', x = 'Work Experience', y = 'YTD Sales (USD)') +
