...
```{r pre-post-declutter, out.width = "100%", fig.cap = "Enhanced design (top). Design defaults (bottom).", fig.align = 'center', warning = FALSE, message = FALSE}
...
## Visualization Types
  ...
  There are some highly advanced and interactive data visualizations that can be built using JavaScript (JS) and D3 libraries. While these are beyond the scope of this book, `ggplot2` is capable of building some very elegant data visualizations, and this will be the tool used to implement each of the data visualizations covered in this section.
...
```{r table, out.width = "100%", fig.cap = ".", fig.align = 'center', warning = FALSE, message = FALSE}
...
### Sankey Diagrams
  ...
  **Sankey diagrams**, or alternatives such as **chord diagrams** and **alluvial plots**, are flow diagrams that are particularly effective in depicting a many-to-many relationship between two domains.
  ...
  Sankey diagrams have many applications in people analytics. For example, sankeys may be used to understand internal transfers over a period of time (i.e., proportion of employees moving from one department to another) or recruiting sources by which employees in various departments have been hired. 
  
  To demonstrate how to implement a sankey diagram, let's use the `sankeyNetwork()` function from the `networkD3` library to visualize employee transfers between departments. While D3 is an advanced JavaScript framework for creating interactive visualizations, the `networkD3` library provides an easy interface for constructing sankey diagrams with interactive components. Building a sankey diagram using `networkD3` requires data to be structured within two data frames:
...
links$value <- ifelse(links$source == links$target-7, round(rnorm(1000, 150, 25), 0), round(rnorm(1000, 30, 5), 0))

```

  With data properly structured within the `nodes` and `links` data frames, these data can be passed into the `sankeyNetwork()` function to construct the sankey diagram:
...
```
  ...
  Though it can quickly become noisy as the number of nodes increases, coloring connections based on the source departments from which they flow may be helpful in tracing the employee flow to the various destination departments. This can be accomplished by adding a grouping variable to the `links` df and passing the variable name to the `LinkGroup` argument in the call to the `sankeyNetwork()` function.
  
  The following code block derives two different grouping variables for use in shading the sankey diagram -- one to indicate the source department (transfers out) and another to indicate intradepartmental (within) vs. interdepartmental (outside) moves.
