...
```{r mlm, out.width = "100%", echo = FALSE, fig.cap = 'Multiple regression of square root transformed YTD sales.', fig.align = 'center'}
...
```
  ...
  Be default, the coefficients on the predictors are unstandardized; that is, they represent the average change in the square root transformed response for each one-unit increase for the respective predictor. Since the predictors have different units of measurement, such as `stock_opt_lvl` ranging from 0 to 3 and `org_tenure` ranging from 0 to 40, the unstandardized coefficients cannot be compared to determine which variable has the largest effect on YTD sales. We must standardize these coefficients and adjust for differences in the units of measurement for an apples-to-apples comparison.

  We can apply the `scale()` function to variables to standardize units of measurement and determine which variable has the largest effect on `ytd_sales`:

```{r mlm-scaled, out.width = "100%", echo = FALSE, fig.cap = 'Multiple regression of YTD sales using scaled variables.', fig.align = 'center'}
...
```
  ...
  Based on the standardized coefficients, `org_tenure` has the largest effect ($\beta$ = .56, $t$(404) = 18.29, $p$ < .001) and `job_lvl` has the second largest effect ($\beta$ = .39, $t$(404) = 12.42, $p$ < .001).
...
```{r mlm-ot, out.width = "100%", echo = FALSE, fig.cap = 'Multiple regression of square root transformed YTD sales for salespeople with overtime.', fig.align = 'center'}
...
```{r mlm-nonot, out.width = "100%", echo = FALSE, fig.cap = 'Multiple regression of square root transformed YTD sales for salespeople without overtime.', fig.align = 'center'}
