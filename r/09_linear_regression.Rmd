...
$$ \hat{y_i} = \hat{\beta}_0 + \hat{\beta_1}x_{i1} + \hat{\beta_2}x_{i2} + {...} + \hat{\beta_p} x_{ip} $$
  ...
  Let's demonstrate the mechanics of multiple linear regression by estimating YTD sales using multiple predictor variables. We can include additional predictors in the model using the `+` symbol in the `lm()` function:
...
```
  ...
  There is a strong relationship between `job_tenure` and `org_tenure`, indicating that job changes for those in sales roles have been infrequent. Let's drop `job_tenure` and reevaluate the correlation coefficients:
...
```
  ...
  Since $|r|$ < .7 for all pairwise relationships, let's fit the more parsimonious multiple regression model:
...
```{r mlm-diagnostics, out.width = "100%", echo = FALSE, fig.cap = 'Multiple Linear Regression Model Diagnostics', fig.align = 'center',}
...
```
  ...
  One initial observation is that there are data points with high leverage on the fit. Let's address using Cook's distance as the criterion:
