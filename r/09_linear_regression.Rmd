...
```{r slm-diagnostics, out.width = "100%", echo = FALSE, fig.cap = 'Linear Regression Model Diagnostics', fig.align = 'center',}
...
```
  ...
  Since $p$ < .05, there is evidence of heteroscedasticity in our data. A common approach to addressing this is transforming the response variable by taking the natural logarithm or square root of the raw data. Let's try addressing heteroscedasticity by log-transforming our response via the `log()` function:
...
```
  ...
  The test statistic for the Breusch-Pagan test is no longer significant ($p$ > .05), indicating the the natural log of YTD sales corrected for the heteroscedasticity observed in the untransformed data. While this is good news, the transformation does result in a less intuitive interpretation of model output which we will cover shortly.
