# Linear Regression {#lm}
  ...
  **Assumptions & Diagnostics**
  ...
  In addition to these key assumptions, a couple of additional diagnostics are important to incorporate into the early data screening stage:
  
  2. **High-Leverage Observations**: Influential data that significantly changes the model fit
  1. **Multicollinearity**: Independent variables that are highly correlated (they should be *independent*)
...
```
  ...
  We can evaluate the 'Residuals vs Fitted' and 'Normal Q-Q' plots in Figure \@ref(fig:slm-diagnostics) to evaluate the linear assumptions of *homoscedasticity* and *linearity*.
...
```
  ...
  Next, we will produce a histogram to visualize the distribution of model residuals ($\hat{y}$ - $y$) to evaluate the assumption of *normality*. In the majority of cases, the residual should be 0 -- that is, in most cases the model correctly estimates YTD sales, in which case there is no difference between estimated and observed values.
