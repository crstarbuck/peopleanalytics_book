# Linear Regression {#lm}
  ...
  **Assumptions & Diagnostics**
  ...
  1. **Multicollinearity**: Independent variables that are highly correlated (these should be *independent*)
...
```
  ...
  Based on these visuals, there are clear violations of linear model assumptions that need to be addressed. 
  
  Given the long right tail for the `ytd_sales` distribution shown in Figure \@ref(fig:pre-mlm-diagnostics), let's apply a square root transformation to the response variable:
...
```  
  ...
  Also, the diagnostic plots indicate that are data points with high leverage on the fit. Let's address using Cook's distance as the criterion:
...
```
  ...
  Let's produce a refreshed set of diagnostic plots to evaluate the impact of removing high leverage observations:

```{r post-mlm-diagnostics, out.width = "100%", echo = FALSE, fig.cap = 'Multiple Linear Regression Model Diagnostics', fig.align = 'center',}
...
```
  ...
  **Variable Selection**
  ...
  Next, we need to reduce our model to the subset of predictors with statistically significant relationships with the response variable. Backward Stepwise Selection is a common and simple variable selection procedure, and the steps for this procedure are outlined below:
  ...
  Each predictor in our model has a significant relationship with `ytd_sales`, so further variable reduction is not required.
