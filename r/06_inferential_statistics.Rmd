...
## Confidence Intervals
  ...
  A **Confidence Interval (CI)** is used to measure uncertainty in sample data. If we draw random samples from a population, we can compute a CI  (a range of values) for each sample. Building on the CLT, for a given confidence level (usually 95% or 99%), the specified percent of sample intervals is expected to include the estimated population parameter (usually $\mu$). For example, for a 95% CI we would expect 95 in every 100 intervals across the samples to include the true population parameter.
  ...
  Next, let's validate that our simulated distribution of sample means adheres to the properties of normally distributed data per the Empirical Rule, since the CLT is so fundamental to these concepts:
