...
```
  ...
  * Geometric: special case of the negative binomial distribution that repeats trials until a success is observed (rather than a fixed number of times)
  ...
  A number of functions are available in R to simulate discrete random probability distributions. The precise shape of a distribution depends on the parameters, but we will simulate and visualize these common PMFs to illustrate the general shape of each:
  
```{r discrete-dist, fig.cap = "Discrete Probability Distributions", fig.align = 'center', message = FALSE}
...
# Create helper function to simplify probability distribution visualization
...
p_nbinomial <- build.viz(nbinomial_dist, "discrete", "Negative Binomial")
p_multinomial <- build.viz(multinomial_dist, "discrete", "Multinomial")
p_poisson <- build.viz(poisson_dist, "discrete", "Poisson")
...
```
  ...
  * F: developed to examine variances from random samples taken from two independent normal populations
  ...
  A number of functions are available in R to simulate continuous random probability distributions. While the precise shape of a distribution depends on the parameters, let's simulate and visualize these common PDFs to illustrate the general shape of each:
