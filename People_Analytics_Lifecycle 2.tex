...
##  /var/folders/b1/0nnhbsx55hvfb1b3n83x0qrw0000gn/T//Rtmp64gmk1/downloaded_packages
\end{verbatim}
...
\includegraphics{People_Analytics_Lifecycle_files/figure-latex/unnamed-chunk-50-1.pdf}
...
\includegraphics{People_Analytics_Lifecycle_files/figure-latex/unnamed-chunk-66-1.pdf}
...
\NormalTok{corrplot}\OperatorTok{::}\KeywordTok{corrplot.mixed}\NormalTok{(M, }\DataTypeTok{order =} \StringTok{'AOE'}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\includegraphics{People_Analytics_Lifecycle_files/figure-latex/unnamed-chunk-81-1.pdf}
...
\hypertarget{parametric-vs.nonparametric-tests-1}{%
\section{Parametric vs.~Nonparametric Tests}\label{parametric-vs.nonparametric-tests-1}}

\hypertarget{differences-of-means}{%
\section{Differences of Means}\label{differences-of-means}}

\hypertarget{differences-of-medians}{%
\section{Differences of Medians}\label{differences-of-medians}}
...
\hypertarget{comparing-2-groups}{%
\section{Comparing 2 Groups}\label{comparing-2-groups}}

\hypertarget{comparing-2-independent-groups}{%
\subsection{Comparing 2 Independent Groups}\label{comparing-2-independent-groups}}
...
\hypertarget{comparing-2-related-groups}{%
\subsection{Comparing 2 Related Groups}\label{comparing-2-related-groups}}

\textbf{Paired Samples T-Test}

\hypertarget{comparing-proportions}{%
\subsection{Comparing Proportions}\label{comparing-proportions}}

\textbf{Two Proportion Z-Test}

\textbf{Chi-Square Test}

\hypertarget{comparing-3-groups}{%
\section{Comparing 3+ Groups}\label{comparing-3-groups}}
...
\tightlist
\item
  \textbf{Independent and Identically Distributed (IID) Random Variables}: Observations in each group are independent of each other (independence), and all observations are taken from the same probability distribution (identically distributed)
\item
...
To test whether the assumption of normality is met, we will first produce and review a \textbf{quantile-quantile (Q-Q) plot}. A Q-Q plot compares two probability distributions by plotting their quantiles (data partitioned into equal-sized groups) against each other. It is important to note that the assumption of normality \emph{does not} apply to the distribution of random variables' data but to \emph{normality of residuals}. Residuals in the context of ANOVA represent the difference between the actual values of the continuous DV relative to its mean value for each level of the categorical IV. The assumption of normality requires that the residuals are distributed normally (with a mean = 0) across each of the levels of the categorical IV. In ANOVA, we expect the residuals to be normally distributed around a mean of 0 when the data are normally distributed; the more skewed the data, the larger the average distance of each DV value from the mean.

We will leverage this plot again in Chapter \ref{lm} to test assumptions of linear models. We will use the \texttt{ggqqplot()} function from the \texttt{ggpubr} library after building a simple linear model via the \texttt{lm()} function:

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Load library for normality plots}
\KeywordTok{library}\NormalTok{(ggpubr)}

\CommentTok{# Build a linear model}
\NormalTok{model <-}\StringTok{ }\KeywordTok{lm}\NormalTok{(annual_comp }\OperatorTok{~}\StringTok{ }\NormalTok{job_sat, }\DataTypeTok{data =}\NormalTok{ employees)}

\CommentTok{# Create a Q-Q plot of residuals}
\NormalTok{ggpubr}\OperatorTok{::}\KeywordTok{ggqqplot}\NormalTok{(}\KeywordTok{residuals}\NormalTok{(model))}
\end{Highlighting}
\end{Shaded}

\includegraphics{People_Analytics_Lifecycle_files/figure-latex/unnamed-chunk-136-1.pdf}

An alternative to a visual inspection of normality is the Shapiro-Wilk test (Shapiro \& Wilk, 1965). The null hypothesis for the Shapiro-Wilk test is that the data are normally distributed. The \texttt{shapiro.test()} function in R can be used to perform this test:
...
## W = 0.9588, p-value < 2.2e-16
\end{verbatim}

Since \texttt{p\ \textless{}\ .05}, we reject the null hypothesis, indicating that there is insufficient evidence for the assumption of normality. This should not be surprising based on the deviation from normality we observed in the Q-Q plot.

Because the assumption of normality is violated, we have two main options. First, we can attempt to transform the data so that the residuals using the transformed values are normally distributed. We can also leverage nonparametric alternatives to ANOVA, which are \textbf{distribution-free tests} that do not require the population's distribution to be characterized by certain parameters. While useful when data are nonnormal and resistant to transformation, it is important to note that nonparametric tests are usually less powerful than their parametric counterparts and also require modification of hypotheses since most nonparametric tests about the \emph{median} rather than \emph{mean} centers.
...
\caption{Q-Q Plots of Transformed DVs}\label{fig:qq-plots-trans}
\end{figure}
...
The test statistic associated with Levene's test relates to the null hypothesis that there are no significant differences in variances across the job satisfaction levels. Since \texttt{p\ \textgreater{}\ .05}, we fail to reject this null hypothesis and can assume equal variances.
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Two-way ANOVA investigating mean differences in annual comp by job satisfaction and dept}
\NormalTok{two.way <-}\StringTok{ }\KeywordTok{aov}\NormalTok{(annual_comp }\OperatorTok{~}\StringTok{ }\NormalTok{job_sat }\OperatorTok{+}\StringTok{ }\NormalTok{dept, }\DataTypeTok{data =}\NormalTok{ employees)}
...
\textbf{Cohen's d} is a standardized measure of the difference between two means. Cohen's \(d\) is defined by:
...
\hypertarget{cliffs-delta}{%
\subsection{Cliff's Delta}\label{cliffs-delta}}
