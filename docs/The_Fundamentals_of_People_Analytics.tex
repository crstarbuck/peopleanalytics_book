...
When integrating data within R, the \texttt{tidyverse} provides a more efficient and parsimonious method of joining many data sets using various join types. Within this framework, components are chained together via the \texttt{\textbar{}\textgreater{}} operator. Note that an older alternative, albeit slightly less efficient, \texttt{\%\textgreater{}\%} operator could instead be used. The example below joins nine data sets into a single \texttt{employees} data frame using a left join on the \texttt{employee\_id} column:
...
{\centering \includegraphics[width=0.75\linewidth]{graphics/data_scientist_tasks} 
