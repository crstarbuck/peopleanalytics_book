...
##   cannot open URL 'http://cran.us.r-project.org/bin/macosx/contrib/4.0/car_3.0-12.tgz'
## 
## The downloaded binary packages are in
##  /var/folders/b1/0nnhbsx55hvfb1b3n83x0qrw0000gn/T//RtmpxYrUmu/downloaded_packages
\end{verbatim}
...
Let's illustrate how to transform the response variable with the square root function:

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Transform YTD leads and refit model}
...
The \emph{Normal Q-Q Plot} in Figure \ref{fig:slm-diagnostics} is used to test the assumption of normally distributed model residuals. A perfectly normal distribution of residuals will result in data lying along the line situated at 45-degrees from the \(x\)-axis. Based on a visual inspection, our residuals appear to be normally distributed, as there are only minor departures in the upper and lower ends of the quantile range.

We can also visualize the distribution of model residuals using a histogram. In the majority of cases, the residual should be 0 -- that is, in most cases the model correctly estimates YTD leads, in which case there is no difference between estimated and observed values (\(\hat{y}\) - \(y\) = 0).
...
Based on both the Normal Q-Q Plot and histogram, the residuals conform to the assumption of normality. We can confirm using the Shapiro-Wilk test:
...
Figure \ref{fig:slm-fit} illustrates how the regression equation for this simple linear model (\(y\) = 20.1\(x\) + 5.1) fits the data points for sales employees. The spread of leads at each engagement level indicates that there are other factors that explain variance in leads that need to be accounted for in the regression equation to achieve more accurate estimates. The reduction in the spread of leads for each combination of the predictor values is directly related to an increase in \(R^2\) (explained variance in YTD leads).
...
\caption{Simple Linear Model Fit Line for y = 20.1x + 5.1}\label{fig:slm-fit}
\end{figure}
