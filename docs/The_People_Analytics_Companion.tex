...
##  /var/folders/b1/0nnhbsx55hvfb1b3n83x0qrw0000gn/T//RtmpRrqwkd/downloaded_packages
\end{verbatim}
...
{\centering \includegraphics{The_People_Analytics_Companion_files/figure-latex/unnamed-chunk-166-1} 

}

\caption{Distribution of Model Residuals}\label{fig:unnamed-chunk-166}
\end{figure}
...
Given the long right tail for the \texttt{ytd\_sales} distribution shown in Figure \ref{fig:pre-mlm-diagnostics}, let's apply a square root transformation to the response variable:
...
\caption{Multiple Linear Regression Model Diagnostics}\label{fig:post-mlm-diagnostics}
\end{figure}
...
{\centering \includegraphics{The_People_Analytics_Companion_files/figure-latex/unnamed-chunk-175-1} 

}

\caption{Distribution of Model Residuals}\label{fig:unnamed-chunk-175}
\end{figure}
...
Each predictor in our model has a significant relationship with \texttt{ytd\_sales}, so further variable reduction is not required.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Produce tabular summary for model results using flextable}
\NormalTok{flextable}\OperatorTok{::}\KeywordTok{as_flextable}\NormalTok{(mlm.fit)}
\end{Highlighting}
\end{Shaded}
