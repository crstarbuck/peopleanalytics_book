...
##  /var/folders/b1/0nnhbsx55hvfb1b3n83x0qrw0000gn/T//Rtmp9NVzrG/downloaded_packages
\end{verbatim}
...
Even with these transformations, there is still a clear S-shaped curve about the residuals. Though we cannot proceed with ANOVA due to violated assumptions, the implementation of ANOVA involves the \texttt{aov()} function along with the \texttt{summary()} function to display model output:
...
While mean annual compensation is significantly different by both job satisfaction and stock option level factors, this output alone is not too helpful in understanding the nature of the differences. Relationships of job satisfaction and stock option level with annual compensation are illustrated more effectively in Figure \ref{fig:two-way-factorial}.
...
As we can see, there is a strong negative relationship between job satisfaction level and average annual compensation among employees with the highest stock option level (3). The relationship between job satisfaction level and average annual compensation appears to be negative for employees with other stock option levels as well, albeit much weaker.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# ANOVA investigating interaction between mean differences in annual comp by job satisfaction x stock option level}
\NormalTok{interaction <-}\StringTok{ }\KeywordTok{aov}\NormalTok{(annual_comp }\OperatorTok{~}\StringTok{ }\NormalTok{job_sat }\OperatorTok{*}\StringTok{ }\NormalTok{stock_opt_lvl, }\DataTypeTok{data =}\NormalTok{ employees)}
...
\begin{verbatim}
##                         Df    Sum Sq   Mean Sq F value  Pr(>F)   
## job_sat                  1 1.337e+10 1.337e+10   7.519 0.00618 **
## stock_opt_lvl            1 6.840e+09 6.840e+09   3.847 0.05001 . 
## job_sat:stock_opt_lvl    1 2.952e+08 2.952e+08   0.166 0.68371   
## Residuals             1466 2.606e+12 1.778e+09                   
## ---
## Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1
\end{verbatim}
