...
##  /var/folders/b1/0nnhbsx55hvfb1b3n83x0qrw0000gn/T//Rtmpfwsp6F/downloaded_packages
\end{verbatim}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Set seed for reproducible random numbers}
\KeywordTok{set.seed}\NormalTok{(}\DecValTok{123}\NormalTok{)}

\CommentTok{# Draw random numbers from normal distribution per defined parameters}
\NormalTok{a <-}\StringTok{ }\KeywordTok{data.frame}\NormalTok{(}\DataTypeTok{x =} \KeywordTok{rnorm}\NormalTok{(}\DecValTok{100000}\NormalTok{, }\DecValTok{100}\NormalTok{, }\FloatTok{.5}\NormalTok{))}
\NormalTok{b <-}\StringTok{ }\KeywordTok{data.frame}\NormalTok{(}\DataTypeTok{x =} \KeywordTok{rnorm}\NormalTok{(}\DecValTok{100000}\NormalTok{, }\DecValTok{101}\NormalTok{, }\FloatTok{.5}\NormalTok{))}

\CommentTok{# Label groups and combine within single df}
\NormalTok{a}\OperatorTok{$}\NormalTok{group <-}\StringTok{ 'a'}
\NormalTok{b}\OperatorTok{$}\NormalTok{group <-}\StringTok{ 'b'}
\NormalTok{ab <-}\StringTok{ }\KeywordTok{rbind}\NormalTok{(a, b)}

\CommentTok{# Visualize distributions}
\NormalTok{ggplot2}\OperatorTok{::}\KeywordTok{ggplot}\NormalTok{(ab, }\KeywordTok{aes}\NormalTok{(x, }\DataTypeTok{fill =}\NormalTok{ group)) }\OperatorTok{+}\StringTok{ }
\NormalTok{ggplot2}\OperatorTok{::}\KeywordTok{labs}\NormalTok{(}\DataTypeTok{title =} \StringTok{"Mean Difference = 1 (p < .001)"}\NormalTok{, }\DataTypeTok{x =} \StringTok{"x"}\NormalTok{, }\DataTypeTok{y =} \StringTok{"Density"}\NormalTok{) }\OperatorTok{+}\StringTok{ }
\NormalTok{ggplot2}\OperatorTok{::}\KeywordTok{geom_density}\NormalTok{(}\DataTypeTok{alpha =} \FloatTok{0.6}\NormalTok{) }\OperatorTok{+}
\NormalTok{ggplot2}\OperatorTok{::}\KeywordTok{scale_fill_manual}\NormalTok{( }\DataTypeTok{values =} \KeywordTok{c}\NormalTok{(}\StringTok{"skyblue"}\NormalTok{,}\StringTok{"lightgrey"}\NormalTok{)) }\OperatorTok{+}
\NormalTok{ggplot2}\OperatorTok{::}\KeywordTok{theme_bw}\NormalTok{()}
\end{Highlighting}
\end{Shaded}
...
\caption{Unstandardized Mean Differences of Independent Samples}\label{fig:mean-group-diff}
\end{figure}
...
\hypertarget{more-than-2-groups}{%
\subsection{More Than 2 Groups}\label{more-than-2-groups}}
