...
##  /var/folders/b1/0nnhbsx55hvfb1b3n83x0qrw0000gn/T//Rtmpt3clH8/downloaded_packages
\end{verbatim}
...
{\centering \includegraphics[width=1\linewidth]{/Users/craig.starbuck/Library/Mobile Documents/com~apple~CloudDocs/Documents/People Analytics Book/GitHub/peopleanalytics_lifecycle_book/graphics/dw_erd} 
...
For variables with more than 2 categories, we can leverage the \texttt{model.matrix()} function to one-hot encode them all:
...
\CommentTok{# Determine location with lowest n-count}
\NormalTok{locations <-}\StringTok{ }\NormalTok{demographics }\OperatorTok{%>%}\StringTok{ }\KeywordTok{count}\NormalTok{(location, }\DataTypeTok{sort =} \OtherTok{TRUE}\NormalTok{)}

\CommentTok{# Display counts by location}
\NormalTok{locations}
\end{Highlighting}
\end{Shaded}
...
## 2                HQ 2 - Atlanta 1522
## 3                        Remote  969
## 4 Operations Center - St. Louis  651
\end{verbatim}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Store location with smallest n-count}
\NormalTok{min_location <-}\StringTok{ }\NormalTok{locations[}\KeywordTok{nrow}\NormalTok{(locations), }\StringTok{"location"}\NormalTok{]}
\end{Highlighting}
\end{Shaded}
...
\CommentTok{# Omit location with lowest n-count}
\NormalTok{location_ohe <-}\StringTok{ }\KeywordTok{model.matrix}\NormalTok{(}\OperatorTok{~}\NormalTok{location}\DecValTok{-1}\NormalTok{, }\DataTypeTok{data =}\NormalTok{ demographics[}\OperatorTok{!}\NormalTok{demographics}\OperatorTok{$}\NormalTok{location }\OperatorTok{==}\StringTok{ }\NormalTok{min_location, ])}
\KeywordTok{head}\NormalTok{(location_ohe)}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
##   locationHQ 1 - Austin locationHQ 2 - Atlanta locationRemote
## 1                     1                      0              0
## 2                     1                      0              0
## 3                     1                      0              0
## 4                     1                      0              0
