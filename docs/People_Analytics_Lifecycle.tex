...
##  /var/folders/b1/0nnhbsx55hvfb1b3n83x0qrw0000gn/T//RtmpCBeB7S/downloaded_packages
\end{verbatim}
...
{\centering \includegraphics{People_Analytics_Lifecycle_files/figure-latex/unnamed-chunk-91-1} 
...
Beyond ensuring the data were generated from a random and representative process, as well as following the data screening procedures outlined in Chapter \ref{data-wrang-prep} (e.g., addressing concerning outliers), parametric tests of differences \emph{generally} feature three key assumptions:
...
Discrete data require nonparametric tests since these data do not come from normally distributed populations. The two most commonly used tests to analyze discrete variables are the \emph{Chi-square test} and \emph{Fischer's exact test}. Both tests organize data within 2x2 \textbf{contingency tables} which enables us to understand interrelations between variables. Figure \ref{fig:contingency-tbl} illustrates a 2x2 contingency table for promotion frequency between remote and non-remote workers:
...
The \textbf{Chi-square test} evaluates patterns of observations to determine if categories occur more frequently than we would expect by chance.

The most common strength test when a significant Chi-Square statistic is observed is Cramer's V.

\textbf{Fischer's Exact Test}
...
A \textbf{Paired Samples \(t\)-Test} is used to compare means between pairs of measurements. This test is known by many other names, such as a \textbf{dependent samples} \(t\)\textbf{-test}, \textbf{paired-difference} \(t\)\textbf{-test}, \textbf{matched pairs} \(t\)\textbf{-test}, and \textbf{repeated-samples} \(t\)\textbf{-test}. We can run a paired samples \(t\)-test in R by passing \texttt{paired\ =\ TRUE} as an argument to the same \texttt{t.test()} function used for the independent samples \(t\)-test.
...
\NormalTok{treat_metrics =}\StringTok{ }\KeywordTok{data.frame}\NormalTok{(}\DataTypeTok{pre_ind =} \KeywordTok{numeric}\NormalTok{(}\KeywordTok{length}\NormalTok{(treat_ids)),}
                           \DataTypeTok{rand_num =} \KeywordTok{rnorm}\NormalTok{(}\KeywordTok{length}\NormalTok{(treat_ids), }\DataTypeTok{mean =} \DecValTok{30}\NormalTok{, }\DataTypeTok{sd =} \DecValTok{5}\NormalTok{) }\OperatorTok{*}\StringTok{ }\FloatTok{.001}\NormalTok{,}
...
\NormalTok{ctrl_metrics =}\StringTok{ }\KeywordTok{data.frame}\NormalTok{(}\DataTypeTok{pre_ind =} \KeywordTok{numeric}\NormalTok{(}\KeywordTok{length}\NormalTok{(ctrl_ids)), }
                          \DataTypeTok{rand_num =} \KeywordTok{rnorm}\NormalTok{(}\KeywordTok{length}\NormalTok{(ctrl_ids), }\DataTypeTok{mean =} \DecValTok{15}\NormalTok{, }\DataTypeTok{sd =} \DecValTok{5}\NormalTok{) }\OperatorTok{*}\StringTok{ }\FloatTok{.001}\NormalTok{, }
...
Based on a visual inspection, differences appear to be somewhat normally distributed. Some semblance of normality should not be surprising given random values were sampled from normal distributions to derive post-intervention happiness indices.
...
Let's test for normality using the Shapiro-Wilk test. The null hypothesis for the Shapiro-Wilk test is that the data are normally distributed. The \texttt{shapiro.test()} function can be used to run this test in R using the model residuals:
...
Since \(p\) \textgreater{} .05 for both tests, the assumption of normally distributed differences is not satisfied. Given the data generative process implemented for this example, the departure from normality likely would have been resolved with a slightly larger sample. Since this key assumption is violated, we will need to leverage a nonparametric alternative to the paired samples \(t\)-test.
