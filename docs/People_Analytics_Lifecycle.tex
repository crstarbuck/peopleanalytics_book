...
##  /var/folders/b1/0nnhbsx55hvfb1b3n83x0qrw0000gn/T//RtmpdnTHIX/downloaded_packages
\end{verbatim}
...
\caption{Density plots comparing distributions with a mean of 100 (blue) and 120 (grey) with sample sizes of 100, 1000, and 10000 (rows) and standard deviations of 25, 50, and 75 (columns). Dashed vertical lines represent distribution means.}\label{fig:mean-group-diff}
\end{figure}
...
The \texttt{wilcox.test()} function is used to run this test in R. Let's illustrate by examining whether engagement (an ordinal variable) is significantly different between those who have been promoted and those who have not:

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Create dummy-coded promotion variable}
\NormalTok{employees}\OperatorTok{$}\NormalTok{promo <-}\StringTok{ }\KeywordTok{ifelse}\NormalTok{(employees}\OperatorTok{$}\NormalTok{last_promo }\OperatorTok{==}\StringTok{ }\DecValTok{0}\NormalTok{, }\DecValTok{0}\NormalTok{, }\DecValTok{1}\NormalTok{)}
