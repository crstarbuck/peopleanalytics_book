...
##  /var/folders/b1/0nnhbsx55hvfb1b3n83x0qrw0000gn/T//Rtmp4k4T8B/downloaded_packages
\end{verbatim}
...
To begin a data analysis, we must first access the requisite data. This often involves extracting data from either source systems in which the data are generated or downstream environments such as a \emph{data lake}, \emph{data warehouse}, or \emph{data mart}.
...
\CommentTok{# Since employee_id is a column in multiple tables used in this query, we need to indicate a specific table to include employee_id in the output}
\NormalTok{sql_string <-}\StringTok{ }\KeywordTok{paste}\NormalTok{(}\StringTok{"SELECT}
