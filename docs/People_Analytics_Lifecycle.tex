...
##  /var/folders/b1/0nnhbsx55hvfb1b3n83x0qrw0000gn/T//RtmpIKf6e8/downloaded_packages
\end{verbatim}
...
\NormalTok{demographics <-}\StringTok{ }\KeywordTok{read.csv}\NormalTok{(}\StringTok{"/Users/craig.starbuck/Library/Mobile Documents/com~apple~CloudDocs/Documents/People Analytics Book/GitHub/peopleanalytics_lifecycle_book/data/files/employee_demographics.csv"}\NormalTok{)}
...
{\centering \includegraphics[width=0.75\linewidth]{/Users/craig.starbuck/Library/Mobile Documents/com~apple~CloudDocs/Documents/People Analytics Book/GitHub/peopleanalytics_lifecycle_book/graphics/sql_joins} 
...
\KeywordTok{unique}\NormalTok{(demographics}\OperatorTok{$}\NormalTok{gender)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{demographics}\OperatorTok{$}\NormalTok{gender_ohe <-}\StringTok{ }\KeywordTok{ifelse}\NormalTok{(demographics}\OperatorTok{$}\NormalTok{gender }\OperatorTok{==}\StringTok{ 'Female'}\NormalTok{, }\DecValTok{1}\NormalTok{, }\DecValTok{0}\NormalTok{)}

\CommentTok{# Preview records}
\KeywordTok{head}\NormalTok{(demographics[, }\KeywordTok{c}\NormalTok{(}\StringTok{"employee_id"}\NormalTok{, }\StringTok{"gender_ohe"}\NormalTok{)])}
\end{Highlighting}
\end{Shaded}
...
\CommentTok{# Return counts by location}
\NormalTok{demographics }\OperatorTok{%>%}\StringTok{ }\KeywordTok{count}\NormalTok{(location, }\DataTypeTok{sort =} \OtherTok{TRUE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
## 2          HQ 2 - Atlanta 1522
## 3                  Remote  969
## 4 Data Center - St. Louis  651
\end{verbatim}

By default, the \texttt{model.matrix()} function will produce a matrix of 1/0 values for \(k-1\) categories. The first column in the matrix is an intercept column containing a value of 1 for each row to ensure linear independence, and the default behavior results in the first value of the factor being positioned as the omitted group. For more flexibility over which value is omitted, we can drop the intercept using \texttt{-1} in the first argument passed to \texttt{model.matrix()} and then choose the omitted group in a subsequent step.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Location one-hot encoding}
\NormalTok{location_ohe <-}\StringTok{ }\KeywordTok{model.matrix}\NormalTok{(}\OperatorTok{~}\NormalTok{location}\DecValTok{-1}\NormalTok{, }\DataTypeTok{data =}\NormalTok{ demographics)}

\CommentTok{# Preview data}
\KeywordTok{head}\NormalTok{(location_ohe)}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
##   locationData Center - St. Louis locationHQ 1 - Austin locationHQ 2 - Atlanta
## 1                               0                     1                      0
## 2                               0                     1                      0
## 3                               0                     1                      0
## 4                               0                     1                      0
...
We will drop the location with the lowest \(n\) rather than the more arbitrary method based on the first value of the factor. Since locations are coded as either \(1\) or \(0\), we can use the \texttt{colSums()} function to sum each column and the \texttt{which.min()} function to identify which has the lowest sum (i.e., least common location).

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Drop location with lowest sum (lowest n-count)}
\NormalTok{location_ohe <-}\StringTok{ }\NormalTok{location_ohe[, }\OperatorTok{-}\KeywordTok{which.min}\NormalTok{(}\KeywordTok{colSums}\NormalTok{(location_ohe))]}

\CommentTok{# Combine demographics and matrix containing one-hot encoded locations}
\NormalTok{demographics <-}\StringTok{ }\KeywordTok{cbind}\NormalTok{(demographics, location_ohe)}

\CommentTok{# Drop original location field}
\NormalTok{demographics <-}\StringTok{ }\KeywordTok{subset}\NormalTok{(demographics, }\DataTypeTok{select =} \OperatorTok{-}\KeywordTok{c}\NormalTok{(location))}
\end{Highlighting}
\end{Shaded}
