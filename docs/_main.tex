...
##  /var/folders/b1/0nnhbsx55hvfb1b3n83x0qrw0000gn/T//RtmpRFKUM2/downloaded_packages
\end{verbatim}
...
\hypertarget{stats}{%
\chapter{Statistical Fundamentals}\label{stats}}
...
{\centering \includegraphics[width=13.62in]{/Users/craig.starbuck/Library/Mobile Documents/com~apple~CloudDocs/Documents/People Analytics Book/GitHub/peopleanalytics_lifecycle_book/graphics/skewness} 
...
{\centering \includegraphics[width=19.69in]{/Users/craig.starbuck/Library/Mobile Documents/com~apple~CloudDocs/Documents/People Analytics Book/GitHub/peopleanalytics_lifecycle_book/graphics/kurtosis} 
...
\caption{Proper Applications of Correlation Coefficients}\label{fig:unnamed-chunk-26}
\end{figure}
...
\caption{2x2 Table for Random Variables x and y}\label{fig:unnamed-chunk-27}
\end{figure}
...
\caption{Correlation between Maine Divorce Rate and Margarine Consumption (r = .99)}\label{fig:unnamed-chunk-31}
\end{figure}
...
\caption{Correlation between Mozzarella Cheese Consumption and Civil Engineering Doctorate Conferrals (r = .96)}\label{fig:unnamed-chunk-32}
\end{figure}
...
\caption{The Empirical Rule}\label{fig:unnamed-chunk-33}
\end{figure}
...
{\centering \includegraphics{_main_files/figure-latex/unnamed-chunk-37-1} 

}

\caption{Uniform Population Distribution (N = 1000)}\label{fig:unnamed-chunk-37}
\end{figure}
...
\caption{Distribution of 10,000 Sample Means of Varied Size}\label{fig:unnamed-chunk-38}
\end{figure}
...
{\centering \includegraphics[width=23.89in]{/Users/craig.starbuck/Library/Mobile Documents/com~apple~CloudDocs/Documents/People Analytics Book/GitHub/peopleanalytics_lifecycle_book/graphics/t_distribution} 
...
\caption{Type I and II Errors}\label{fig:unnamed-chunk-53}
\end{figure}
...
It is at this juncture that Monty introduces a twist by asking if the contestant would like to switch doors. Most assume that the two closed doors have an equal (50/50) chance of containing the prize, because we generally think of probabilities as independent, random events. However, this is incorrect. Contestants who switch from their original selection to the other closed door double their chances of winning. This may be counterintuitive, because the brain wants to reduce the problem to a simple coin flip. There is a major difference between the Monty Hall problem and a coin flip; in order for two outcomes to have the same probability, randomness and independence are required. In the case of the Monty Hall problem, neither assumption is satisfied.

When all three doors are closed, each has the same probability of being selected. The probability of choosing the door with a prize is .33. This is because the choice is completely random given we have no information that would increase the probability of a door containing the prize. The process is no longer random when Monty uses his insider knowledge about the prize's location and opens a door he knows does not contain the prize. Since Monty will never show the door containing the prize, he is careful to always open a door that has nothing behind it. If he was not constrained by the requirement to not reveal the prizes's location and instead chose to open one of the remaining doors at random, the probabilities would be equal (and he may end up opening the door that contains the prize).

Let's prove this with a simulation in R:
...
\CommentTok{# Calculate percent of wins for switch vs. keep decisions}
...
\KeywordTok{round}\NormalTok{((switch_wins }\OperatorTok{-}\StringTok{ }\NormalTok{keep_wins) }\OperatorTok{/}\StringTok{ }\NormalTok{switch_wins }\OperatorTok{*}\StringTok{ }\DecValTok{100}\NormalTok{, }\DecValTok{0}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
As we can see, wins occur 33\% more often when contestants switch doors. If ever you find yourself playing Let's Make a Deal, switch doors.
s
\#\# Exercises
...
\begin{figure}
\includegraphics[width=25.81in]{/Users/craig.starbuck/Library/Mobile Documents/com~apple~CloudDocs/Documents/People Analytics Book/GitHub/peopleanalytics_lifecycle_book/graphics/4d_framework} \caption{Figure 1: 4D Framework}\label{fig:unnamed-chunk-55}
\end{figure}
