...
##  /var/folders/b1/0nnhbsx55hvfb1b3n83x0qrw0000gn/T//Rtmp3ma3nG/downloaded_packages
\end{verbatim}
...
The goal of research is to understand a population based on data from a subset of population members. In practice, it is often not feasible to collect data from every member of a population, so we instead calculate \textbf{sample statistics} -- such as people who participate in a survey -- in order to estimate \textbf{population parameters}.
...
There are several correlation coefficients, and the measurement scale of \texttt{x} and \texttt{y} determine the appropriate type:

\begin{figure}

{\centering \includegraphics[width=24.39in]{/Users/craig.starbuck/Library/Mobile Documents/com~apple~CloudDocs/Documents/People Analytics Book/GitHub/peopleanalytics_lifecycle_book/graphics/correlation_table} 
...
The \(\rho\) coefficient of .67 indicates that the positive association we observed between job level and education level is strong. We could also pass \texttt{method\ =\ "kendall"} to this cor() function to implement Kendall's \(\tau\).
...
To illustrate, let's examine whether there is a relationship between gender and performance after transforming performance from its ordinal form to a dichotomous variables (high vs.~low performance). We can leverage the \texttt{psych} library to calculate \(\phi\) in R:
...
\caption{Uniform Population Distribution (N = 1000)}\label{fig:unnamed-chunk-31}
\end{figure}
...
\caption{Distribution of 100 Sample Means of Varied Size}\label{fig:unnamed-chunk-32}
\end{figure}
...
\caption{Critical Values of Student's t Distribution}\label{fig:unnamed-chunk-41}
\end{figure}
...
\caption{Type I and II Errors}\label{fig:unnamed-chunk-48}
\end{figure}
...
\begin{figure}
\includegraphics[width=25.81in]{/Users/craig.starbuck/Library/Mobile Documents/com~apple~CloudDocs/Documents/People Analytics Book/GitHub/peopleanalytics_lifecycle_book/graphics/4d_framework} \caption{Figure 1: 4D Framework}\label{fig:unnamed-chunk-49}
\end{figure}
