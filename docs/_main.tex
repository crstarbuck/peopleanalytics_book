...
##  /var/folders/b1/0nnhbsx55hvfb1b3n83x0qrw0000gn/T//RtmpCEfKAO/downloaded_packages
\end{verbatim}
...
\StringTok{  }\KeywordTok{labs}\NormalTok{(}\DataTypeTok{x =} \StringTok{"Organization Tenure"}\NormalTok{, }\DataTypeTok{y =} \StringTok{"n"}\NormalTok{) }\OperatorTok{+}\StringTok{ }
...
\caption{2x2 Table for Random Variables x and y}\label{fig:unnamed-chunk-28}
\end{figure}
...
\caption{Correlation between Maine Divorce Rate and Margarine Consumption (r = .99)}\label{fig:unnamed-chunk-32}
\end{figure}
...
\caption{Correlation between Mozzarella Cheese Consumption and Civil Engineering Doctorate Conferrals (r = .96)}\label{fig:unnamed-chunk-33}
\end{figure}
...
\caption{The Empirical Rule}\label{fig:unnamed-chunk-34}
\end{figure}
...
\caption{Distribution of 100 Sample Means of Varied Size}\label{fig:unnamed-chunk-39}
\end{figure}
...
Now, let's illustrate the relationship between CIs and standard errors using sample data from our uniform population distribution. In our example, both \(\mu\) and \(\sigma\) are known and our sample size \(n\) is at least 30; therefore, we can use a \textbf{Z-Test} to calculate the 95\% CI. A \(z\) score of 1.96 corresponds to the 95\% CI for a two-tailed distribution (i.e., we are looking for significantly different values in either the larger or smaller direction) -- the range of values we would expect to include \(\mu\) in at least 95 of 100 random samples taken from the population:
...
Let's compare CIs calculated using a T-Test to those calculated using the Z-Test. While a fixed \(z\) score can be used for each CI level when \(n\) \textgreater{} 30, the \(t\) statistic varies based on both the CI level and \(df\). We can reference the following table to obtain the \(t\) statistic for each CI level:
...
\caption{Critical Values of Student's t Distribution}\label{fig:unnamed-chunk-48}
\end{figure}
...
\caption{Type I and II Errors}\label{fig:unnamed-chunk-55}
\end{figure}
...
\begin{figure}
\includegraphics[width=25.81in]{/Users/craig.starbuck/Library/Mobile Documents/com~apple~CloudDocs/Documents/People Analytics Book/GitHub/peopleanalytics_lifecycle_book/graphics/4d_framework} \caption{Figure 1: 4D Framework}\label{fig:unnamed-chunk-56}
\end{figure}
