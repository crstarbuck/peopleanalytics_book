...
\KeywordTok{install.packages}\NormalTok{(}\KeywordTok{c}\NormalTok{(}\StringTok{"tidyverse"}\NormalTok{, }\StringTok{"corrplot"}\NormalTok{), }\DataTypeTok{dependencies =} \OtherTok{TRUE}\NormalTok{, }\DataTypeTok{repos =} \StringTok{"http://cran.us.r-project.org"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
##  /var/folders/b1/0nnhbsx55hvfb1b3n83x0qrw0000gn/T//RtmpzEGdsk/downloaded_packages
\end{verbatim}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Read employee tenure data}
\NormalTok{tenure <-}\StringTok{ }\KeywordTok{read.csv}\NormalTok{(}\StringTok{"https://raw.githubusercontent.com/crstarbuck/peopleanalytics_lifecycle_book/master/data/tables/tbl_tenure.csv"}\NormalTok{)}

\CommentTok{# Calculate sample variance for total working years}
\KeywordTok{var}\NormalTok{(tenure}\OperatorTok{$}\NormalTok{total_working_years)}
\end{Highlighting}
\end{Shaded}
...
Sample statistics are the default in R. Since the population variance differs from the sample variance by a factor of \texttt{var(vector)\ *\ (n\ -\ 1)\ /\ n}, it is simple to convert output from var() to the population variance:
...
\CommentTok{# Calculate population variance for total working years}
\KeywordTok{var}\NormalTok{(tenure}\OperatorTok{$}\NormalTok{total_working_years) }\OperatorTok{*}\StringTok{ }\NormalTok{(n }\OperatorTok{-}\StringTok{ }\DecValTok{1}\NormalTok{) }\OperatorTok{/}\StringTok{ }\NormalTok{n}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Calculate sample standard deviation for total working years}
\KeywordTok{sd}\NormalTok{(tenure}\OperatorTok{$}\NormalTok{total_working_years)}
\end{Highlighting}
\end{Shaded}
...
Since the population standard deviation differs from the sample standard deviation by a factor of \texttt{sd(vector)\ *\ sqrt((n\ -\ 1)\ /\ n)}, it is simple to convert output from sd() to the population standard deviation:

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Calculate population standard deviation for total working years}
\KeywordTok{sd}\NormalTok{(tenure}\OperatorTok{$}\NormalTok{total_working_years) }\OperatorTok{*}\StringTok{ }\KeywordTok{sqrt}\NormalTok{((n }\OperatorTok{-}\StringTok{ }\DecValTok{1}\NormalTok{) }\OperatorTok{/}\StringTok{ }\NormalTok{n)}
\end{Highlighting}
\end{Shaded}
...
The Interquartile Range (IQR) represents the difference between Q3 and Q1 cutpoint values (the middle two quartiles). The IQR is sometimes used to detect extreme values in a distribution. Values less than \texttt{Q1\ -\ 1.5\ *\ IQR} or greater than \texttt{Q3\ +\ 1.5\ *\ IQR} are generally considered outliers.

In R, the quantile() function returns the values that bookend each quartile:

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Return quartiles for total working years}
\KeywordTok{quantile}\NormalTok{(tenure}\OperatorTok{$}\NormalTok{total_working_years)}
\end{Highlighting}
\end{Shaded}
...
We can return a specific percentile value using the \texttt{probs} argument in the quantile() function:

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Return 80th percentile total working years value}
\KeywordTok{quantile}\NormalTok{(tenure}\OperatorTok{$}\NormalTok{total_working_years, }\DataTypeTok{probs =} \FloatTok{.8}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\KeywordTok{summary}\NormalTok{(tenure}\OperatorTok{$}\NormalTok{total_working_years)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Calculate covariance between total working years and age using complete observations (missing values will cause issues if not addressed)}
\KeywordTok{cov}\NormalTok{(tenure}\OperatorTok{$}\NormalTok{total_working_years, tenure}\OperatorTok{$}\NormalTok{years_at_company, }\DataTypeTok{use =} \StringTok{"complete.obs"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
In this example, using the default Pearson method, the covariance between total working years and age is 453179.1. Since the value is positive, we know that total working years is generally higher for older employees and lower for younger employees.

Since total working years and age are both measured on continuous scales, the default Pearson method for evaluating the linear relationship is appropriate. However, people analytics often involves working with ordinal non-linear data (e.g., survey data measured on Likert-type scales, numerically coded education levels), and for these data the Spearman method for rank-ordered variables should be used. We can override the default method using the \texttt{method} argument in the cov() function call:

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Read survey response data}
\NormalTok{responses <-}\StringTok{ }\KeywordTok{read.csv}\NormalTok{(}\StringTok{"https://raw.githubusercontent.com/crstarbuck/peopleanalytics_lifecycle_book/master/data/tables/tbl_survey_responses.csv"}\NormalTok{)}

\CommentTok{# Calculate covariance between job satisfaction and work-life balance scores using Spearman's method}
\KeywordTok{cov}\NormalTok{(responses}\OperatorTok{$}\NormalTok{job_satisfaction, responses}\OperatorTok{$}\NormalTok{relationship_satisfaction, }\DataTypeTok{method =} \StringTok{"spearman"}\NormalTok{, }\DataTypeTok{use =} \StringTok{"complete.obs"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
While covariance is an unstandardized measure of the direction of a relationship between variables, correlation provides a standardized measure that can be used to quantify both the direction and strength of the relationship. Correlation coefficients range from -1 to 1, where -1 indicates a perfectly negative relationship, 1 indicates a perfectly positive relationship, and 0 indicates the absence of a relationship. Pearson's correlation coefficient \(r\) is defined by:
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Calculate the correlation between total working years and years at company}
\KeywordTok{cor}\NormalTok{(tenure}\OperatorTok{$}\NormalTok{total_working_years, tenure}\OperatorTok{$}\NormalTok{years_at_company, }\DataTypeTok{use =} \StringTok{"complete.obs"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
While we knew the relationship between total working years and years at company is positive based on the positive covariance coefficient, Pearson's \(r\) of .8 indicates that the strength of the positive association is strong (\(r\) = 1 is perfectly positive).

As we've covered, Pearson's \(r\) is not appropriate for non-linear ordinal data. For this, we can leverage Spearman's \(\rho\), which is a standardized nonparametric measure of the relationship between two rank-ordered variables. Spearman's \(\rho\) is defined as:

\[ \rho = 1- {\frac {6 \sum d_i^2}{n(n^2 - 1)}} \]

Like the cov() function, we can override the default Pearson method in the cor() function using the \texttt{method} argument:

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Calculate the correlation between Belonging and Culture scores using Spearman's method}
\KeywordTok{cor}\NormalTok{(responses}\OperatorTok{$}\NormalTok{job_satisfaction, responses}\OperatorTok{$}\NormalTok{relationship_satisfaction, }\DataTypeTok{method =} \StringTok{"spearman"}\NormalTok{, }\DataTypeTok{use =} \StringTok{"complete.obs"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
The \(\rho\) coefficient indicates that the negative relationship between job satisfaction and relationship satisfaction is quite weak relative to the relationship we observed between total working years and years at company. Nevertheless, there is some degree of negative covariance between these variables.

Rather than looking at individual pairwise relationships, we can easily produce a correlation matrix for many variables.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Generate a correlation matrix between response columns}
\KeywordTok{cor}\NormalTok{(responses[, }\DecValTok{2}\OperatorTok{:}\DecValTok{5}\NormalTok{], }\DataTypeTok{method =} \StringTok{"spearman"}\NormalTok{, }\DataTypeTok{use =} \StringTok{"complete.obs"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
## worklife_balance                        0.005766111      1.000000000
\end{verbatim}

Based on this correlation matrix, there is little or no relationship between responses on these four survey variables. The values down the diagonal are 1 because these represent the correlation between each variable and itself. You may also notice that the information above and below the diagonal is identical and, therefore, redundant.

A great library for visualizing correlation matrices is corrplot. Several arguments can be specified to surface the coefficient and various visual representations of the relationships among variables. The \texttt{hclust} argument orders variables based on similarities in the strength of relationships:
...
\NormalTok{M <-}\StringTok{ }\KeywordTok{cor}\NormalTok{(tenure[, }\DecValTok{2}\OperatorTok{:}\DecValTok{6}\NormalTok{], }\DataTypeTok{method =} \StringTok{"spearman"}\NormalTok{, }\DataTypeTok{use =} \StringTok{"complete.obs"}\NormalTok{)}

\CommentTok{# Visualize correlation matrix}
\KeywordTok{corrplot}\NormalTok{(M, }\DataTypeTok{method =} \StringTok{'shade'}\NormalTok{, }\DataTypeTok{type =} \StringTok{"upper"}\NormalTok{, }\DataTypeTok{order =} \StringTok{"hclust"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\includegraphics{_main_files/figure-latex/unnamed-chunk-22-1.pdf}

It's important to remember that correlation is not causation. Correlations can be spurious (variables related by chance), and drawing conclusions based on correlations -- especially without drawing on theory -- can be dangerous. Here are two examples of nearly perfect associations between variables that are likely related only by chance:

\begin{figure}

{\centering \includegraphics[width=13.33in]{/Users/craig.starbuck/Library/Mobile Documents/com~apple~CloudDocs/Documents/People Analytics Book/GitHub/peopleanalytics_lifecycle_book/graphics/spurious_corr_maine_divorce} 

}

\caption{Correlation between Maine Divorce Rate and Margarine Consumption}\label{fig:unnamed-chunk-23}
\end{figure}

\begin{figure}

{\centering \includegraphics[width=13.33in]{/Users/craig.starbuck/Library/Mobile Documents/com~apple~CloudDocs/Documents/People Analytics Book/GitHub/peopleanalytics_lifecycle_book/graphics/spurious_corr_mozzarella_cheese} 

}

\caption{Correlation between Mozzarella Cheese Consumption and Civil Engineering Doctorate Conferrals}\label{fig:unnamed-chunk-24}
\end{figure}
...
\caption{The Empirical Rule}\label{fig:unnamed-chunk-25}
\end{figure}
...
{\centering \includegraphics{_main_files/figure-latex/unnamed-chunk-28-1} 

}

\caption{Uniform Population Distribution (N = 1000)}\label{fig:unnamed-chunk-28}
\end{figure}
...
\caption{Distribution of 100 Sample Means of Varied Size}\label{fig:unnamed-chunk-29}
\end{figure}
...
\NormalTok{ci95_lower_bound <-}\StringTok{ }\KeywordTok{mean}\NormalTok{(x_bars) }\OperatorTok{-}\StringTok{ }\FloatTok{1.96} \OperatorTok{*}\StringTok{ }\NormalTok{(}\KeywordTok{sd}\NormalTok{(x_bars) }\OperatorTok{/}\StringTok{ }\KeywordTok{sqrt}\NormalTok{(n))}
\NormalTok{ci95_upper_bound <-}\StringTok{ }\KeywordTok{mean}\NormalTok{(x_bars) }\OperatorTok{+}\StringTok{ }\FloatTok{1.96} \OperatorTok{*}\StringTok{ }\NormalTok{(}\KeywordTok{sd}\NormalTok{(x_bars) }\OperatorTok{/}\StringTok{ }\KeywordTok{sqrt}\NormalTok{(n))}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{ci99_lower_bound <-}\StringTok{ }\KeywordTok{mean}\NormalTok{(x_bars) }\OperatorTok{-}\StringTok{ }\FloatTok{2.58} \OperatorTok{*}\StringTok{ }\NormalTok{(}\KeywordTok{sd}\NormalTok{(x_bars) }\OperatorTok{/}\StringTok{ }\KeywordTok{sqrt}\NormalTok{(n))}
\NormalTok{ci99_upper_bound <-}\StringTok{ }\KeywordTok{mean}\NormalTok{(x_bars) }\OperatorTok{+}\StringTok{ }\FloatTok{2.58} \OperatorTok{*}\StringTok{ }\NormalTok{(}\KeywordTok{sd}\NormalTok{(x_bars) }\OperatorTok{/}\StringTok{ }\KeywordTok{sqrt}\NormalTok{(n))}
\end{Highlighting}
\end{Shaded}
...
\caption{Type I and II Errors}\label{fig:unnamed-chunk-38}
\end{figure}
...
\begin{figure}
\includegraphics[width=25.81in]{/Users/craig.starbuck/Library/Mobile Documents/com~apple~CloudDocs/Documents/People Analytics Book/GitHub/peopleanalytics_lifecycle_book/graphics/4d_framework} \caption{Figure 1: 4D Framework}\label{fig:unnamed-chunk-39}
\end{figure}
