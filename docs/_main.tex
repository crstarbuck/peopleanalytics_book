...
##  /var/folders/b1/0nnhbsx55hvfb1b3n83x0qrw0000gn/T//RtmpgmBwTn/downloaded_packages
\end{verbatim}
...
The examples thus far have only examined associations between two variables at a time. However, rather than looking at isolated pairwise relationships, we can produce a covariance matrix to surface associations among many variables:
...
Using the default Pearson method, the cov() function will return sample variances for each variable down the diagonal, since the concept of covariance in the context of a variable with itself is not applicable. We can confirm by producing the variance for age:

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Return sample variance for annual compensation}
