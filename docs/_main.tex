...
##  /var/folders/b1/0nnhbsx55hvfb1b3n83x0qrw0000gn/T//RtmpesTRZU/downloaded_packages
\end{verbatim}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Calculate covariance between annual compensation and age using complete observations (missing values will cause issues if not addressed)}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Calculate covariance between job level and education level}
\KeywordTok{cov}\NormalTok{(demographics}\OperatorTok{$}\NormalTok{job_level, demographics}\OperatorTok{$}\NormalTok{education, }\DataTypeTok{method =} \StringTok{"spearman"}\NormalTok{, }\DataTypeTok{use =} \StringTok{"complete.obs"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
The positive covariance coefficient indicates that those with more education tend to achieve higher levels in the organization relative to those with less education.
...
As we've covered, Pearson's \(r\) is not appropriate for non-linear ordinal data. For this, we can leverage Spearman's \(\rho\), which is a standardized nonparametric measure of the association between two rank-ordered variables. Spearman's \(\rho\) is defined as:
...
The \(\rho\) coefficient of .67 indicates that the positive association we observed between job level and education level is strong.

Rather than looking at individual pairwise relationships, we can easily produce a correlation matrix for many variables.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Generate a correlation matrix between continuous variables}
...
A great library for visualizing correlation matrices is corrplot. Several arguments can be specified for various visual representations of the relationships among variables:
...
\caption{Correlation between Maine Divorce Rate and Margarine Consumption (r = .99)}\label{fig:unnamed-chunk-23}
\end{figure}
...
\caption{Correlation between Mozzarella Cheese Consumption and Civil Engineering Doctorate Conferrals (r = .96)}\label{fig:unnamed-chunk-24}
\end{figure}
